...
const contractAddress = '0x80bA15DCd21E3260ddF573DAAA59Cccd442C2109'; 
...
function App() {
 ...
 const handleConnectWallet = async () => {
  ...
  if (!localProvider) {
    if (window.ethereum) {
      localProvider = new ethers.providers.Web3Provider(window.ethereum, 'any');
  ...
  try {
    ...
    if (accounts.length > 0) {
      setCurrentAccount(accounts[0]);
      const newSigner = localProvider.getSigner();
      setSigner(newSigner);
